...
async function startServer() {
  ...
  app.post("/api/events/pitchdeck", authenticate, upload.single('pitchdeck'), (req: any, res) => {
    if (!req.file) return res.status(400).json({ error: "No file uploaded" });
    const pitchDeckUrl = `/uploads/${req.file.filename}`;
    // If the client also sends an eventId, we could save it directly, but typically 
    // the frontend will upload and then supply the URL when saving the event.
  ...
  app.post("/api/events", authenticate, (req: any, res) => {
    ...
    const stmt = db.prepare(`
      INSERT INTO events (
        id, title, description, date, time, location, venueName, format, 
        imageUrl, price, category, hostId, sponsorId, attendees, capacity, 
        tags, isFeatured, sponsorshipSettings, visibility, pitchDeckUrl, createdAt, updatedAt
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    `);
  ...
  app.put("/api/events/:id", authenticate, (req: any, res) => {
    ...
    const stmt = db.prepare(`
      UPDATE events SET 
        ...
        capacity = ?, tags = ?, isFeatured = ?, sponsorshipSettings = ?, visibility = ?, pitchDeckUrl = ?, updatedAt = CURRENT_TIMESTAMP
      WHERE id = ?
    `);
