...
import { 
  ...
  Tag, Briefcase, FileText, Mail, Home, Compass, Shield
} from 'lucide-react';
import { EVENTS, SPONSORS, MOCK_USER } from './constants';
import { Event, Sponsor, TicketSelection, ChatMessage, EventFormat, EventSponsorshipSettings, SponsorshipItem, AudienceType, SponsorshipProposal } from './types';
...
const ProfilePage = () => {
    const { user, events, proposals } = useEvents();
    ...
    return (
        <div className="min-h-screen pt-24 px-4 pb-32 max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-12 text-center md:text-left">
                <img src={user.avatarUrl || 'https://picsum.photos/seed/user/200'} className="w-24 h-24 rounded-full border-2 border-purple-500 p-1" />
                <div>
            ...
            <div className="flex gap-4 mb-8 border-b border-white/10">
                <button 
                    ...
                    className={`pb-4 px-2 text-sm font-bold transition-all relative ${activeTab === 'events' ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    Events & Tickets
                    {activeTab === 'events' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-500" />}
                </button>
                <button 
                    onClick={() => setActiveTab('proposals')}
                    className={`pb-4 px-2 text-sm font-bold transition-all relative ${activeTab === 'proposals' ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    Sponsorship Proposals
                    {(receivedProposals.length > 0) && <span className="ml-2 bg-purple-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">{receivedProposals.length}</span>}
                    {activeTab === 'proposals' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-500" />}
                </button>
            </div>
                                ...
                                {hostedEvents.map(event => (
                                    <div key={event.id} className="glass-card p-4 rounded-2xl flex gap-4 hover:bg-white/5 transition-colors">
                                         ...
                                         <div className="min-w-0">
                                             ...
                                             <div className="mt-3">
                                                 <Link 
                                                    to={`/event/${event.id}/sponsor/settings`}
                        ...
                        {receivedProposals.length > 0 ? (
                            <div className="space-y-4">
                                {receivedProposals.map(proposal => (
                                    <div key={proposal.id} className="glass-card p-6 rounded-3xl border border-white/10 space-y-4">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <h3 className="font-bold text-white text-lg">{proposal.senderName}</h3>
                                                <p className="text-sm text-gray-400">wants to sponsor <span className="text-purple-400 font-bold">{proposal.eventTitle}</span></p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-xl font-bold text-white">${proposal.estimatedInvestment}</p>
                                                <p className="text-[10px] text-gray-500 uppercase tracking-wider">{new Date(proposal.timestamp).toLocaleDateString()}</p>
                                            </div>
                                        </div>
                                        <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                                            <p className="text-sm text-gray-300 italic leading-relaxed whitespace-pre-wrap">"{proposal.message}"</p>
                                        </div>
                                        <div className="flex gap-3">
                                            <Button variant="glass" className="text-xs flex-1">Decline</Button>
                                            <Button variant="primary" className="text-xs flex-1">Accept & Chat</Button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="text-center py-12 bg-white/5 rounded-3xl border border-dashed border-white/10">
                                <p className="text-gray-500">No sponsorship proposals received yet.</p>
                            </div>
                        )}
                    </div>
                        ...
                        {sentProposals.length > 0 ? (
                            <div className="space-y-4">
                                {sentProposals.map(proposal => (
                                    <div key={proposal.id} className="glass-card p-6 rounded-3xl border border-white/10 space-y-4 opacity-80 hover:opacity-100 transition-opacity">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <h3 className="font-bold text-white text-lg">Proposal for {proposal.eventTitle}</h3>
                                                <p className="text-sm text-gray-400">Status: <span className="text-yellow-500 font-bold uppercase tracking-widest text-[10px]">{proposal.status}</span></p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-xl font-bold text-white">${proposal.estimatedInvestment}</p>
                                                <p className="text-[10px] text-gray-500 uppercase tracking-wider">{new Date(proposal.timestamp).toLocaleDateString()}</p>
                                        ...
                                        <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                                            <p className="text-sm text-gray-400 italic leading-relaxed whitespace-pre-wrap">"{proposal.message}"</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="text-center py-12 bg-white/5 rounded-3xl border border-dashed border-white/10">
                                <p className="text-gray-500">You haven't sent any sponsorship proposals yet.</p>
                            </div>
                        )}
                    </div>
                </div>
...
const App = () => {
  ...
  return (
    <EventContext.Provider value={{ events, addEvent, user: MOCK_USER, proposals, addProposal }}>
      <HashRouter>
